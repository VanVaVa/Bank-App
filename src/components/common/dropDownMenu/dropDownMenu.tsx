...
type DropDownMenuProps = {
  ...
  accent?: boolean;
  defaultValue: string;
  param: keyof CardType;
  onChange: (currentValue: string, param: keyof CardType) => void;
};
...
export const DropDownMenu: React.FC<DropDownMenuProps> = ({
  ...
  const listOptions = options.map((option: string) => (
    <li className="dropdown_element" key={option}>
      <DropDownElement
        $accent={accent}
      ...
      >
        ...
        >
          <rect
            ...
            width="20"
            height="4"
            transform="rotate(45 2.82837 11.3137)"
          />
          <rect
            x="25.4558"
            y="11.3137"
            width="4"
            height="20"
            transform="rotate(45 25.4558 11.3137)"
          />
        </StyledSvg>
      </DropDownElement>
      <DropdownList $displayed={opened} $accent={accent}>
        {listOptions}
      </DropdownList>
    </DropdownWrapper>
  );
...
const DropdownWrapper = styled.span<{ $accent: boolean }>`
  background-color: ${(props) =>
    props.$accent ? props.theme.accent : props.theme.light};
